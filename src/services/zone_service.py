...
class ZoneService:
    ...
    def update_zone_data(self, base_id: int, zone_id: int, request: ZoneRequest) -> Tuple[bool, str]:
        """
        ...
        if request.state is not None:
            ...
            try:
                ...
                if success:
                    _logger.info(f"Write-through state to physical device successful: {msg}")
                    ...
                    if "verified after timeout" in msg.lower():
                        ...
                        _logger.warning(f"State write succeeded but required verification due to timeout: {msg}")
                else:
                    _logger.warning(f"Write-through state to physical device failed: {msg}")
                    # Log the specific error for debugging
                    _logger.warning(f"Write-through state error details: {msg}")
            except Exception as e:
                _logger.error(f"Error during write-through state to physical device: {e}")
            ...
            if not write_through_success:
                _logger.warning("Write-through state to physical device failed. Data saved locally but may not persist on device restart.")
                _logger.warning("Check Waveshare gateway connection and Neasmart device communication.")
            elif write_through_verified:
                _logger.info("Write-through state succeeded with verification. Device communication may be slow but functional.")
