...
4. **State Enrichment**: States/modes must be provided as human-readable strings (integers are not accepted)
...
1. **Update Zones API** (`/zones/{base_id}/{zone_id}`):
   - **GET Response** - Enriched zone data matching OpenAPI spec:
     ...
     ```
   - **POST Request** - Accept only state strings (integers are not accepted):
     ```json
     {
       "state": "presence",
       "setpoint": 22.5
...
2. **Update Operation API** (`/operation/state`, `/operation/mode`):
   - **GET /operation/state** - Return enriched state data:
     ...
     ```
   - **POST /operation/state** - Accept only state name (integers are not accepted):
     ```json
     {
       "state": "presence"
     }
